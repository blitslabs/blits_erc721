...
export async function getHarmonyContract(abi: any[], address?: string, options?: ContractOptions): Promise<HarmonyContract>{
    if(!contracts[address]){
        contracts[address] = (await getHarmonyExtension()).contracts.createContract(abi, address, options);
    }
